...
\begin{examplebreak}[Partial functions]
    If $X$ and $Y$ are sets, then we order the set $\pmaps{X}{Y}$ of partial functions as follows: For $f,g \in \pmaps{X}{Y}$ we let $f \leq g$ if $\dom f \subseteq \dom g$ and $f(x) = g(x)$ for all $x \in \dom f$. The \keyword{graph} of a partial function $f$ is the set
    %
    \begin{equation*}
    ...
    \begin{equation*}
        \dom \bigdjoin D
            = \pi_X\image[\big]{\graph{\bigdjoin D}}
...
We begin by studying dcpo's. Let $P$ be a dcppo and $F \colon P \to P$ a monotone function. We clearly have $\bot \leq F(\bot)$, and since $F$ is monotone we get the chain\footnote{A \keyword{chain} is a totally ordered set.}
%
\begin{equation*}
    \bot
        \leq F(\bot)
...
\begin{theorem}[Principle of rule induction]
    ...
    If $\calR$ is a set of inference rules on $X$ and $A \subseteq X$, then $\mu \calR \subseteq A$ if the following condition holds: For every inference rule $R \in \calR$ with antecedents lying in $A$, the consequent of $R$ also lies in $A$.
\end{theorem}
...
\begin{proposition}
    ...
    If $F$ is represented by $\calR$ and $y \in X$, then $y \in \mu F$ if and only if $y$ is the final conclusion of some closed derivation from $\calR$. [TODO only in terms of R]
\end{proposition}
...
\begin{corollary}[Inversion]
    \label{cor:inversion}
    If $\calR$ is a collection of inference rules and $y \in \mu \calR$, then there is a rule in $\calR$ with $y$ as consequent whose antecedents also lie in $\mu \calR$.
\end{corollary}

\begin{proof}
    By \cref{prop:fixpoint-iff-derivation} there is a closed derivation $D$ of $y$ from $\calR$, and we denote its last rule by $R$. If $x$ is an antecedent of $R$, then some subderivation of $D$ is a derivation of $x$ by \cref{enum:subderivation}. Another application of \cref{prop:fixpoint-iff-derivation} then implies that $x \in \mu \calR$.
\end{proof}
...
\begin{theorem}[Definition by recursion]
    Let $\calR$ be a set of inference rules [TODO need uniqueness!], and let $Z$ be any set. For each $R \in \calR$ let $h_R \colon \mu \calR \prod Z^{\rho(R)} \to Z$ be a function [TODO define arity of rule]. Then there is a unique function $f \colon \mu \calR \to Z$ such that if $x_1, \ldots, x_k$ are the antecedents and $y$ the consequent of a rule $R$, then
    %
    \begin{equation*}
        f(y)
            = h_R(y, f(x_1), \ldots, f(x_k)).
    \end{equation*}
\end{theorem}

\begin{proof}
    Define a function $G \colon \pmaps{\mu\calR}{Z} \to \pmaps{\mu\calR}{Z}$ as follows: For $f \in \pmaps{\mu\calR}{Z}$ let $\dom G(f)$ be the set of elements $y \in \mu\calR$ such that if $R \in \calR$ is the unique rule with consequent $y$, then the antecedents of $R$ lie in $\dom f$. For $y \in \dom G(f)$ we then let
    %
    \begin{equation*}
        G(f)(y)
            = h_R(y, f(x_1), \ldots, f(x_k)).
    ...
    \begin{align*}
        \dom G(\bigdjoin D)
            &= F(\dom \bigdjoin D)
             = F(\bigdjoin \dom \image{D}) \\
            &= \bigdjoin F\image{\dom \image{D}}
             = \bigdjoin \dom \image{G\image{D}} \\
            &= \dom \bigdjoin G\image{D}.
    \end{align*}
    %
    For $y$ in this common domain there is a $d \in D$ such that $y \in \dom G(d)$. If $R$ is the unique rule with consequent $y$ and $x_1, \ldots, x_k$ are its antecedents, then these lie in $\dom d$, and so
    %
    \begin{align*}
        G(\bigdjoin D)(y)
            &= h_R(y, (\bigdjoin D)(x_1), \ldots, (\bigdjoin D)(x_k)) \\
            &= h_R(y, d(x_1), \ldots, d(x_k)) \\
            &= G(d)(y) \\
            &= (\bigdjoin G\image{D})(y).
    \end{align*}
    %
    ...
    \end{equation*}
    %
    Hence $\mu G$ is fact a total function solving the equation [TODO ref], which proves the theorem.
\end{proof}
...
\end{equation*}
%
[TODO formalise expression] That is, for now we ignore sorts and simply ... [TODO without fresh renamings etc., are UASTs even necessary?]
